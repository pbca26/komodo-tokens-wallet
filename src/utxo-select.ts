...
const getUtxosRawTx = async (utxos: any) => {
  let rawTx = [];
...
export const utxoSelectNormal = (address: string, value: number, ccLibFormat: boolean, ccLibVersion: number) => {
...
export const utxoSelectCC = (address: string, tokenId: string, ccLibFormat: boolean, ccLibVersion: number) => {
  return new Promise(async(resolve, reject) => {
    let utxos: any = [];
    
    for (let a = 0; a < address.length; a++) {
      utxos = utxos.concat(JSON.parse(JSON.stringify(await blockchain.tokenUtxos(address[a], tokenId, true))));
    }
