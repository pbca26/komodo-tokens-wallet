...
import {BuyTokenModalProps} from './types';

const BuyTokenModal: React.FC<BuyTokenModalProps> = props => {
      ...
      try {
        ...
        inputsData = {
          ...
          normalUtxos: await Blockchain.createCCTx(
            toSats(state.price * state.amount + 0.00001),
            address.pubkey
          ),
        };
  ...
  const dropdownClickHandler = (tokenInfo: any, tokenListItems: any) => {
    if (tokenInfo.height > -1)
    setToken({
      balance: tokenListItems.supply,
      tokenId: tokenListItems.tokenid,
      name: tokenInfo.name
    });
  }
        ...
        if (!state.dropdownQuickSearch ||
          ...
          items.push(
            <a
              ...
              onClick={() => dropdownClickHandler(tokenInfo, tokenListItems[i])}>
              {tokenInfo.name}
              {tokenInfo.height > 0 &&
