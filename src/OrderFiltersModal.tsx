...
import {OrderFiltersModalProps} from './types';

const OrderFiltersModal: React.FC<OrderFiltersModalProps> = props => {
  const [isClosed, setIsClosed] = useState(true);
  const [tokenDropdownOpen, setTokenDropdownOpen] = useState(-1);
  
  const dropdownEventHandler = (e: React.MouseEvent<HTMLButtonElement>, index: number) => {
    e.stopPropagation();

    setTokenDropdownOpen(index === tokenDropdownOpen ? -1 : index);
  };
