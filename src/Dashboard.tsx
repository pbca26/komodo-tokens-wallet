...
const Dashboard: React.FC<DashboardProps> = props => {
  ...
  const setActiveToken = (activeToken: any) => {
    setState((prevState: any) => ({
      ...prevState,
  ...
  const syncData = async () => {
    ...
    const tokenBalance = await Blockchain.tokenBalance(chains[chain].isRaddress ? address.normal : address.cc);
    const tokenTransactions = await Blockchain.tokenTransactions(chains[chain].isRaddress ? address.normal : address.cc);
    ...
    setState((prevState: any) => ({
      ...
      tokenBalance: tokenBalance.balance.filter((x: any) => x.balance > 0),
  ...
  const renderTokenInfo = () => {
    if (state.activeToken) {
      ...
      writeLog('tokenInfo', tokenInfo);

      const checkTypeOfArbitraryData = (data: any) => {
        try {
          JSON.parse(data);
          writeLog('JSON.parse(data)', JSON.parse(data));
          return true;
        } catch (e) {
          writeLog(e)
        }
