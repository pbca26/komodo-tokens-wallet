...
const SendTokenModal: React.FC<SendTokenModalProps> = props => {
  ...
  const sendToken = async () => {
    if (Number(state.amount) > state.token.balance ||
      ...
      try {
        let inputsData, rawtx, r2cindex;
          
        writeLog('create token modal txBuilderApi', chains[chain].txBuilderApi);

        if (chains[chain].isRaddress) {
          const {cIndexKeys} = await Blockchain.raddressToCIndexKeys(address.normal);
          writeLog('r2cindex', cIndexKeys)
          r2cindex = cIndexKeys;
        }
        ...
        if (chains[chain].txBuilderApi === 'utxoSelect') {
          inputsData = {
            ccUtxos: await utxoSelectCC(
              chains[chain].isRaddress ? r2cindex : [address.cc],
              state.token.tokenId,
              true,
          ...
          <div className="create-token-form">
            ...
            <div className="input-form">
              ...
              <input
                type="text"
                name="pubkey"
                placeholder={`Destination ${chains[props.chain].isRaddress ? 'address' : 'pubkey'}`}
                value={state.pubkey}
                onChange={updateInput}
